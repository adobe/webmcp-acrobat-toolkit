...
async function updateBadge(tabId) {
  ...
  if (!tab || tab.id !== tabId) return; // no active tab in this window → nothing to badge
...
chrome.runtime.onMessage.addListener(({ tools }, sender) => {
  // Messages from the side panel / extension pages have no sender.tab — guard against tab.id crash.
